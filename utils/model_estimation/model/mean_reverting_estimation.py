from utils.calc_var_ABC import VaRCalculationMethod, SharedCacheCopulaMRVaR
from kalman_mean_reverting.optimize import VolOptimizer
from kalman_mean_reverting.estimate import KalmanFilterVolEstimation
from kalman_mean_reverting.forecast import calc_forecast
import numpy as np
import pandas as pd
...
class MeanRevertingEstimation(VaRCalculationMethod):
    """
    ...
    def model_params_insample(in_sample_dict):
        ...
        for ticker, returns in in_sample_dict.items():
            ...
            cache_key = (ticker)
            if cache_key in SharedCacheCopulaMRVaR.cache:
                results_dict[ticker] = SharedCacheCopulaMRVaR.cache[cache_key]
            ...
            params, LL = optimizer.em_algorithm(returns)

            # Store the results
            results_dict[ticker] = {
                'optimal_params': {'a': params[0], 'l': params[1], 'q': params[2]}
            }

            # Cache the result
            SharedCacheCopulaMRVaR.cache[cache_key] = results_dict[ticker]
    ...
    def calculate_marginals_and_densities_in_sample(in_sample_dict, in_sample_params):
        ...
        for ticker, params in in_sample_params.items():
            ...
            if cache_key in SharedCacheCopulaMRVaR.cache:
                cached_result = SharedCacheCopulaMRVaR.cache[cache_key]
            ...
            a = params['optimal_params']['a']
            l = params['optimal_params']['l']
            q = params['optimal_params']['q']
            returns = in_sample_dict[ticker]

            likelihood = KalmanFilterVolEstimation(a, l, q, l, q, len(returns), returns)

            eps = likelihood.calc_eps_t()

            marginals = norm_cdf_array(eps)
            densities = norm_pdf_array(eps)

            # Cache marginals, densities, and vol_states separately
            SharedCacheCopulaMRVaR.cache[cache_key] = {
                'marginals': marginals,
                'densities': densities,
            }
        ...
        for ticker, params in in_sample_params.items():
            ...
            for date, ticker_dict in rolling_windows_dict.items():
                ...
                if cache_key in SharedCacheCopulaMRVaR.cache:
                    ...
                    forecasts = calc_forecast(returns, a, l, q)
